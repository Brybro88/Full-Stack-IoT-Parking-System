...
const char *DATABASE_URL =
    "https://parking-iot-a0b8e-default-rtdb.firebaseio.com/";
...
void loop() {
  ...
  if (now - lastSensorRead >= SENSOR_READ_INTERVAL) {
    ...
    Serial.printf("📏 Dist: %.1f cm | 🌡 Temp: %.1f°C | 💧 Hum: %.1f%% | 🎤 "
                  "Sonido: %d (Avg: %.0f) | 🅿️ Estado: %s | 🔊 Ruido: %s\n",
                  distancia_cm, temperatura_c, humedad_pct, nivel_sonido,
                  ruidoBaselineAvg, estadoActual.c_str(), estadoRuido.c_str());
  ...
  bool estadoCambiado =
      (estadoActual != estadoAnterior && estadoActual != "Inicializando");
  bool tiempoDeEnvio = (now - lastFirebaseSend >= FIREBASE_SEND_INTERVAL);

  if ((estadoCambiado || tiempoDeEnvio) && Firebase.ready() &&
      WiFi.status() == WL_CONNECTED) {
    lastFirebaseSend = now;
    enviarAFirebase();
  } else if ((estadoCambiado || tiempoDeEnvio) && !Firebase.ready()) {
    Serial.println(
        "❌ ERROR: Intento de envío fallido. Firebase.ready() es FALSO.");
...
void iniciarFirebase() {
  ...
  if (Firebase.ready()) {
    ...
    Serial.println(
        "❌ ERROR FATAL al iniciar Firebase. Revisa credenciales y REGLAS.");
    Serial.print("     Razón: ");
